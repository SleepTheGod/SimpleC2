            ...
            if request == "exit":
                ...
                with open(filename, 'wb') as f:
                    ...
                    while data:
                        f.write(data)
                        data = cipher_suite.decrypt(sock.recv(1024))
                    sock.send(cipher_suite.encrypt(b"File uploaded successfully"))
...
tcp_thread.daemon = True  # Ensures the thread terminates when the main process exits
tcp_thread.start()

if __name__ == '__main__':
    print_banner()
    app.run(host='0.0.0.0', port=5000, threaded=True)  # Added threaded=True for Flask to handle requests concurrently
